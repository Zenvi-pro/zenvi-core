# Graph Report - c:\msys64\home\ranje\zenvi-core  (2026-07-20)

## Corpus Check
- Large corpus: 237 files · ~1,196,989 words. Semantic extraction will be expensive (many Claude tokens). Consider running on a subfolder.

## Summary
- 4916 nodes · 9575 edges · 282 communities (241 shown, 41 thin omitted)
- Extraction: 83% EXTRACTED · 17% INFERRED · 0% AMBIGUOUS · INFERRED: 1610 edges (avg confidence: 0.69)
...
- `get_timeline_state()` --calls--> `get_source_window()`  [INFERRED]
  src/classes/tool_handlers.py → src/classes/ai_metadata_utils.py
...
Cohesion: 0.02
Nodes (86): QMenu, Zenvi Backend API Client.  Drop-in replacement for direct AI class imports. Th, @file  @brief This file creates the QApplication, and displays the main window, @file  @brief Background auto-updater that checks GitHub releases and downloads, @file  @brief This file loads and saves settings (as JSON)  @author Noah Figg <e, @file  @brief This file connects to libopenshot and logs debug messages (if deb, @file  @brief This file sets the default logging settings  @author Noah Figg <, Adjust the minimum log level written to our logfile (+78 more)

### Community 1 - "Classes Tool Handlers"
Cohesion: 0.04
Nodes (105): add_clip_to_timeline(), add_marker(), add_stock_media_to_project(), add_track(), add_transition_between_clips(), add_transition_to_clip(), add_tts_audio_to_timeline(), apply_transition() (+97 more)
...
Cohesion: 0.04
Nodes (71): Fa(), Aa(), ad(), Af(), ag(), Bf(), Ca(), cb() (+63 more)

### Community 4 - "Windows Main Window 2"
Cohesion: 0.03
Nodes (35): get_app(), Get the current QApplication instance of OpenShot, Save the object back to the project data store, get_audio_data(), Get a Clip object form libopenshot, and grab audio data         For for the give, CosmicTheme, Toggle the play icon between play and pause., Handle when playback is started (+27 more)

### Community 5 - "Windows Views 2"
Cohesion: 0.04
Nodes (42): BackgroundPainter, QPainter, QRectF, @file  @brief Painter for the timeline background gradient.  @author Jonathan Th, BasePainter, @file  @brief Base painter helpers for the QWidget timeline backend.  @author Jo, Return ``(width, height)`` of *pixmap* in logical units., Return *pixmap* scaled to the requested logical size. (+34 more)

### Community 6 - "Windows Classes"
Cohesion: 0.04
Nodes (44): QItemDelegate, ClipboardManager, @file  @brief This file is responsible for serializing copy/paste clipboard data, Manages clipboard operations for QueryObjects or lists of QueryObjects, Converts QueryObject or list of QueryObjects to QMimeData.         Handles both, Converts QMimeData back into the original object (QueryObject or list of QueryOb, Clip, Effect (+36 more)
...
Cohesion: 0.05
Nodes (41): get_assets_path(), @file  @brief This file generates the path for a project's assets  @author Jonat, Get and/or create the current assets path. This path is used for thumbnail and b, change_profile(), @file  @brief This file converts project data from one FPS to a new FPS (adjusti, Adjust all clip-like objects to use project FPS precision, adjusting 'end' trim, remove_gaps(), KeyframeScaler (+33 more)

### Community 9 - "Windows Views 4"
Cohesion: 0.04
Nodes (22): A Web(Engine/Kit)View QWidget used to load the Timeline, Hide the waveform for the selected clip, Begin a keyframe drag operation, Callback when thumbnail needs to be updated, Finalize a keyframe drag operation and record history, Callback for copy context menus, Callback for removing gap context menus, Callback for removing all gaps on a layer starting from the detected gap (+14 more)
...
Cohesion: 0.08
Nodes (22): AIChatWindow, QDockWidget, Apply restored /chat/history data into in-memory + visible UI., Render a stored (role, html_body) tuple into the widget chat box., Clear and re-render stored messages for the active session (widget mode only)., Rebuild the widget fallback multi-chat tab bar., Run JavaScript in the embedded WebEngine or WebKit chat page., Push theme colors, models, preamble and welcome message to the CEP UI. (+14 more)
...
Cohesion: 0.09
Nodes (38): _candidate_from_context(), _check_ambiguity(), ClipCandidate, _effective_min_confidence(), _fmt_position_mmss(), _format_candidates_error(), _metadata_corpus(), _normalize_text() (+30 more)

### Community 17 - "Timeline Media 3"
Cohesion: 0.11
Nodes (35): ab(), B(), bb(), ea(), Eb(), $f(), fa(), fb() (+27 more)

### Community 18 - "Windows Title Editor"
Cohesion: 0.08
Nodes (22): dict_to_style(), @file  @brief Utility functions for manipulating SVG style attributes  @author F, Explode an SVG node style= attribute string into a dict representation, Turn an exploded style dictionary back into a string, set_if_existing(), style_to_dict(), QColor, QDialog (+14 more)
...
Cohesion: 0.05
Nodes (20): Set up model/view classes for MainWindow, EffectsModel, EffectsProxyModel, QObject, QSortFilterProxyModel, Filter for common transitions and text filter, EmojisModel, EmojiStandardItemModel (+12 more)
...
Cohesion: 0.17
Nodes (33): a(), b(), ba(), c(), d(), E(), f(), Ga() (+25 more)
...
Cohesion: 0.08
Nodes (15): QPointF, QWidget, Return (QTransform, unpacked props, originScreenPt) for a clip/effect box., Signal to refresh viewport (i.e. a property might have changed that effects the, Callback for resize event timer (to delay the resize event, and prevent lots of, Resize zoom button clicked, Clear all transform-related state to avoid using invalid clip/effect objects, A QWidget used on the video display widget (+7 more)

### Community 28 - "Classes Ai Metadata Utils"
Cohesion: 0.09
Nodes (35): adjust_scene_descriptions_for_subclip(), apply_metadata_to_clip_data(), build_tags_preview(), clip_metadata_is_valid(), collect_scene_descriptions_for_baked_segment(), _filter_scenes_in_window(), _filter_tags_for_window(), filter_tags_string_for_window() (+27 more)

### Community 29 - "Windows Views 10"
Cohesion: 0.07
Nodes (12): QWidget, Clear all timeline selections and keyframe highlights., Estimate content height for size hint calculations., Enable or disable snapping mode., Enable or disable razor tool mode., Enable or disable timing (retime) mode., Placeholder due to webview compatibility, Placeholder due to webview compatibility (+4 more)
...
Cohesion: 0.09
Nodes (25): _clip_media_path(), _db_to_volume(), export_edl(), _fmt_percent(), _fmt_value(), _interp_name(), _is_drop_frame(), @file  @brief This file is used to generate an EDL (edit decision list) export (+17 more)

### Community 32 - "Windows Main Window 4"
Cohesion: 0.07
Nodes (17): Callback for max sized change (i.e. max size of video widget), This class syncs changes from the timeline to libopenshot, This method is invoked by the UpdateManager each time a change happens (i.e Upda, TimelineSync, get_icon(), Get either the current theme icon or fallback to default theme (for custom icons, Get a list of all dockable widgets, Freeze/unfreeze a dock widget on the main screen. (+9 more)

### Community 33 - "Timeline Media 6"
Cohesion: 0.09
Nodes (16): $a(), ac(), cd(), Da(), Ec(), Ef(), Hf(), If() (+8 more)

### Community 34 - "Windows Ai Chat Ui 2"
Cohesion: 0.07
Nodes (23): AIChatWorker, _join_content_blocks(), _markdown_to_html(), _parse_content_blocks(), _plain_to_html(), QObject, Normalize an assistant reply into a plain markdown string.      Some providers, Convert markdown to HTML suitable for QTextEdit. Uses theme text color for body. (+15 more)
...
Cohesion: 0.09
Nodes (30): _enumerate_timeline_clips(), Legacy tuple rows for tests patching this hook., Take any arguments given as filters, and find the first matching object, build_timeline_clip_context(), _cached_file_data(), clear_metadata_lookup_cache(), enumerate_timeline_contexts(), Any (+22 more)
...
Cohesion: 0.07
Nodes (17): GetThumbPath(), Get thumbnail path by invoking HTTP thumbnail request, QFrame, The label to display selections, Create and display the selection menu when requested., SelectionLabel, QObject, @file  @brief Lightweight thumbnail request manager for the QWidget timeline bac (+9 more)

### Community 40 - "Classes Twelvelabs Match"
Cohesion: 0.10
Nodes (31): _fmt_mmss(), get_clips_with_full_metadata(), _parse_explicit_source_time_range_sec(), _parse_mmss_or_hhmmss_token(), Return a source-file cut time from scene descriptions, or None., Return seconds for 'SS', 'M:SS', or 'H:M:SS' tokens, else None., If *query* names a concrete time range in source seconds, return (t0, t1)., Return project files with AI metadata for planning (excludes hidden subclips by (+23 more)

### Community 41 - "Windows Ai Chat Ui 3"
Cohesion: 0.09
Nodes (9): ChatBridge, Ground the model with a bounded timeline snapshot (main thread)., Start a background thread to summarize the first user prompt and update preamble, Shared send pipeline for web and widget chat UIs., Handle send from CEP UI (same logic as send_message but with args)., Run deterministic plan executor via backend., Switch to Plan mode and prefill chat to revise a failed step., QWebChannel bridge: exposes sendMessage, cancelRequest, clearChat to the CEP cha (+1 more)
...
Cohesion: 0.10
Nodes (14): AIMediaPanel, QDockWidget, Refresh the scenes tree (based on current selection)., Filter scene descriptions by search text., Listen for file selection changes to show per-clip tags., Return (timeline_clip, file_obj, file_id) for the dock display context., Resolve ai_metadata for the current display target., Show or hide indexing progress bar and status label. (+6 more)

### Community 46 - "Classes Tool Handlers 2"
Cohesion: 0.11
Nodes (25): get_backend_client(), Get the singleton backend client (refreshed when backend URL changes)., charge_operation_on_success(), check_operation(), build_project_index_name(), collect_project_twelvelabs_index(), map_search_hit_to_file(), Any (+17 more)

### Community 47 - "Classes Exporters 2"
Cohesion: 0.11
Nodes (27): _append_link(), _apply_rate_settings(), _apply_timecode_settings(), _clip_file_info(), _displayformat(), export_xml(), _format_ratio(), _format_timebase() (+19 more)

### Community 48 - "Classes Metrics"
Cohesion: 0.08
Nodes (22): _base_event_params(), _build_event(), _d(), Build a GA4 event payload., Track a GUI screen being shown, Track an error has occurred, Track application session start/end., Send anonymous GA4 Measurement Protocol events over HTTP. (+14 more)

### Community 49 - "Classes Track Display"
Cohesion: 0.10
Nodes (28): build_editor_snapshot_for_chat(), delete_clips_on_track(), _file_is_analyzed(), get_timeline_state(), list_layers(), Return a structured snapshot of the current timeline: tracks, clips with positio, Compact timeline + file count for LLM grounding. Call from Qt GUI thread., Delete all clips on a UI track (Track 1..N bottom=1) or storage layer_number. (+20 more)

### Community 50 - "Timeline Media 7"
Cohesion: 0.32
Nodes (28): $c(), ue(), a(), b(), C(), d(), e(), f() (+20 more)
...
Cohesion: 0.09
Nodes (13): CreditsFilterProxyModel, CreditsModel, CreditsStandardItemModel, QSortFilterProxyModel, QStandardItemModel, @file  @brief This file contains the credits model, used by the about window  @a, Proxy class used for sorting and filtering credits, Match filter against name, email, or website columns (+5 more)
...
Cohesion: 0.10
Nodes (11): Return a cached, detached copy of child, clearing cache when project changes., Take any arguments given as filters, and find a list of matching objects, Get the translated display title of this item, Get the translated display title of this item, Get AI metadata for this file, Check if file has been analyzed by AI, Get AI-generated tags, Get AI-generated description (+3 more)
...
Cohesion: 0.21
Nodes (15): Ba(), db(), Df(), Ia(), Jc(), Kc(), lb(), Mc() (+7 more)

### Community 65 - "Windows Export"
Cohesion: 0.11
Nodes (11): Export, _parse_bitrate_to_bps(), QDialog, Start exporting video, Convert a bitrate value to integer bits-per-second.     Accepts int, float, or s, Resolve requested audio codec to one that is available on this system.     Preve, Restore defaults by closing and reopening the dialog., Update progress bar during exporting (+3 more)
...
Cohesion: 0.12
Nodes (9): Preferences, QDialog, Update the Restore Defaults button label based on the selected tab., Apply dependency state to all registered widgets., Enable/disable dependent widgets based on controller state., Check if the app needs to restart, Push the current thumbnail preference to the QWidget timeline., Test specific settings for hardware decode (+1 more)
...
Cohesion: 0.11
Nodes (15): center(), connect_auto_events(), init_element(), init_ui(), load_icon_theme(), load_ui(), @file  @brief This file contains PyQt help functions, to translate the interfac, Using the window xml, set the icon on the given element,     or if theme_name p (+7 more)
...
Cohesion: 0.11
Nodes (7): _DownloadWorker, Any, QFrame, A single result card: thumbnail + duration badge + hover overlay., Downloads a single Pexels video on a background thread., Release per-video thread/worker refs after QThread::finished., _VideoCard
...
Cohesion: 0.11
Nodes (8): Callback for move up button click, Handle callback for image length being changed, Calculate the total length of what's about to be added to the timeline, TimelineModel, QTreeView, @file  @brief This file contains the add to timeline file treeview  @author Jona, A TreeView QWidget used on the add to timeline window, TimelineTreeView
...
Cohesion: 0.14
Nodes (9): humanize_tool_name(), Return a short human-readable title for a tool name., _format_tool_command(), Render a Cursor-style collapsible terminal block for a tool call., Append one log line to a running tool block., Mark a tool block as done/error and keep result text for inspection., Build a `$`-style preview line summarising the tool invocation., Collapsible tool-run block for native Qt chat (mirrors chat.js tool blocks). (+1 more)

### Community 92 - "Timeline Media 9"
Cohesion: 0.14
Nodes (13): $d(), Dc(), Gd(), Kf(), of(), Pa(), pf(), q() (+5 more)

### Community 93 - "Windows About"
Cohesion: 0.12
Nodes (8): About, Handle right-click context menu., Copy cleaned version info (without HTML <div> and unnecessary links) to clipboar, Get the current version, Get the current version # from openshot.org, Load Credits for everybody who has contributed in several domain for Openshot, Load License of the project, Load the changelog window

### Community 94 - "Windows Freesound Dock"
Cohesion: 0.17
Nodes (5): FreesoundDock, QDockWidget, Freesound stock-music/SFX search dock., Runs freesound_search on a background thread., _SearchWorker
...
Cohesion: 0.16
Nodes (7): PexelsDock, QDockWidget, Pexels stock-video search dock for Zenvi.  Search bar (QLineEdit + magnifying-, Pexels stock-video search dock., Runs pexels_search on a background thread., # NOTE: do NOT touch self._dl_threads/_dl_workers here — the QThread, _SearchWorker
...
Cohesion: 0.12
Nodes (8): BlockPaintFilter, draw_checkerboard(), PickingDialog, PreviewFrameFilter, QDialog, Draw a checkerboard pattern for transparent backgrounds., Intercepts paint events on the QColorDialog’s preview frame,     draws a checker, Use an external editor to edit the image

### Community 101 - "Windows Freesound Dock 2"
Cohesion: 0.14
Nodes (10): _DownloadWorker, QObject, QPixmap, QRunnable, Freesound stock-music/SFX search dock for Zenvi.  Search bar (QLineEdit + magn, Downloads a single Freesound MP3 preview on a background thread., Fetches a waveform image URL and emits the result via a signal carrier., _Signals (+2 more)
...
Cohesion: 0.18
Nodes (6): FilesModel, QObject, Attach AI metadata to a file object (does not save)., Queue a file for background tagging with bounded concurrency., Fire-and-forget background AI tagging for an already-saved file., Update/re-generate the thumbnail of a specific file
...
Cohesion: 0.21
Nodes (13): Formatter that trims overly long log messages., TruncatingFormatter, copyFileToFolder(), framesInClip(), isClip(), makeLegalFileName(), nameOfExport(), notClip() (+5 more)
...
Cohesion: 0.15
Nodes (8): ChangelogFilterProxyModel, ChangelogModel, ChangelogStandardItemModel, QSortFilterProxyModel, QStandardItemModel, @file  @brief This file contains the git changelog model, used by the about wind, Proxy class used for sorting and filtering changelog entries, Format columns and sort

### Community 113 - "Windows Preferences 2"
Cohesion: 0.14
Nodes (7): textChanged event handler for search box, Delete all tabs and ensure they are fully removed from memory., Populate all preferences and tabs, Store widget references and register dependency relationships., Prompt the user for confirmation before restoring defaults for the current tab., Check all keyboard settings for duplicate or invalid shortcuts and update the UI, Update the UI to provide feedback for duplicated/invalid shortcuts.
...
Cohesion: 0.20
Nodes (8): QLabel, Return a flat section-header label that replaces QGroupBox titles., Create the tags browser tab, _section_header(), Any, QFrame, QWidget, Zenvi logo + wordmark for login pages.
...
Cohesion: 0.14
Nodes (7): Delete the object from the project data store, Delete the object from the project data store, Delete the object from the project data store, Delete the object from the project data store, Delete the object from the project data store, Delete the object from the project data store, Delete the object from the project data store
...
Cohesion: 0.15
Nodes (8): Thread-safe session for parallel upload + tagging requests., Index via presigned TwelveLabs upload (proxy encoded locally)., Poll /indexing/job/{job_id} until the job finishes or max_wait seconds pass., Re-index via direct TwelveLabs presigned upload., Any, Upload a local video file to TwelveLabs presigned URLs (no backend video hop)., Upload file chunks; return (parts, error). parts match upload-complete schema., upload_file_via_presigned_urls()
...
Cohesion: 0.23
Nodes (6): FileProperties, QDialog, Initialize the start and end textboxes based on a file object, If the path has changed, verify that path is valid, and         update duration,, File Properties Dialog, Init all form elements / textboxes / etc...

### Community 129 - "Windows Freesound Dock 3"
Cohesion: 0.18
Nodes (3): QFrame, A single result card: waveform + name + duration badge + hover overlay., _SoundCard

### Community 130 - "Windows Views 36"
Cohesion: 0.19
Nodes (8): QObject, @file  @brief This file contains the titles model, used by the title editor wind, TitleRoles, TitlesModel, QListView, @file  @brief This file contains the titles treeview, used by the title editor w, A QListView QWidget used on the title editor window, TitlesListView
...
Cohesion: 0.20
Nodes (5): QPalette, make_dark_palette(), clipExportWindow, QDialog, A popup to export clips as mp4 files     in a folder of the user's choosing

### Community 134 - "Classes Query 3"
Cohesion: 0.17
Nodes (6): Save the object back to the project data store, Save the object back to the project data store, Save the object back to the project data store, Save the object back to the project data store, Save the object back to the project data store, Save the object back to the project data store
...
Cohesion: 0.24
Nodes (9): _extract_frame_jpeg(), extract_tagging_frames(), _ffmpeg_run(), Extract JPEG frames from local video files for backend tagging (ffmpeg)., Extract one downscaled JPEG frame at *timestamp* seconds., Return (frames, error). Each frame is (timestamp_seconds, jpeg_bytes)., get_tagging_interval(), Tagging frame interval logic (mirrors zenvi-backend gemini_provider.get_tagging_ (+1 more)
...
Cohesion: 0.20
Nodes (8): _get_dispatcher(), _MainThreadDispatcher, QObject, Re-run AI tagging for an existing project file.      Runs on a worker thread., Singleton helper that runs callables on the Qt main (GUI) thread., Headless fallback when PyQt5 is unavailable., Return (and lazily create) the singleton main-thread dispatcher., retag_project_file()
...
Cohesion: 0.22
Nodes (6): QSortFilterProxyModel, QStandardItemModel, Proxy class used for sorting and filtering model data, Sort titles model by a column at runtime, TitleFilterProxyModel, TitleStandardItemModel

### Community 163 - "Windows Pexels Dock 3"
Cohesion: 0.24
Nodes (6): QObject, QPixmap, QRunnable, Fetches a thumbnail URL and emits the result via a signal carrier., _Signals, _ThumbnailRunnable
...
Cohesion: 0.29
Nodes (4): QTreeView, A TreeView QWidget used on the main window, Override startDrag method to display custom icon, TransitionsTreeView
...
Cohesion: 0.29
Nodes (5): BackendTaggingWorker, QThread, Close the active HTTP session to unblock any pending request., Called at app quit — interrupt any pending HTTP requests, then wait briefly., Background worker that calls the zenvi-backend tagging API.
...
Cohesion: 0.33
Nodes (3): Download a public CDN URL to a local temp file on the desktop., Download a Pexels MP4 from the CDN URL to the local machine., Download a Freesound preview MP3 from the CDN URL to the local machine.
...
### Community 208 - "Classes Ui Util 2"
Cohesion: 0.33
Nodes (6): Search for transitions by name., search_transitions(), get_default_icon(), Get a QIcon, and fallback to default theme if OS does not support themes., Search for theme name, search_dir()
...
### Community 214 - "Windows Views 60"
Cohesion: 0.33
...
### Community 215 - "Windows Views 61"
Cohesion: 0.33
Nodes (3): Invoke razor slicing when the razor tool is enabled and an item is clicked., Convert an x position in widget coordinates to timeline seconds., Callback from javascript that the razor tool was clicked
...
- **Why does `get_app()` connect `Windows Main Window 2` to `Windows Views`, `Classes Tool Handlers`, `Windows Main Window`, `Windows Views 2`, `Windows Classes`, `Windows Views 3`, `Classes Project Data`, `Windows Views 4`, `Classes Clip Utils`, `Windows Ai Chat Ui`, `Windows Views 6`, `Classes Clip Resolver`, `Windows Title Editor`, `Windows Main Window 3`, `Windows Models`, `Windows Views 7`, `Windows Views 8`, `Windows Views 9`, `Classes App`, `Windows Video Widget`, `Windows Views 10`, `Windows Views 11`, `Classes Exporters`, `Windows Main Window 4`, `Classes Timeline Clip Context`, `Classes Thumbnail`, `Windows Views 12`, `Windows Views 13`, `Windows Ai Media Panel`, `Classes Tool Handlers 2`, `Classes Exporters 2`, `Classes Metrics`, `Windows Preview Thread`, `Windows Views 17`, `Windows Models 2`, `Classes Auto Updater`, `Classes Importers`, `Windows Views 19`, `Classes Query`, `Windows Export`, `Classes Api Client`, `Classes Importers 2`, `Windows Cutting`, `Windows Preferences`, `Windows Profile Edit`, `Classes Ui Util`, `Windows Views 20`, `Windows Views 21`, `Windows Pexels Dock`, `Windows Views 22`, `Windows Views 23`, `Windows Views 24`, `Windows Views 27`, `Windows Views 28`, `Windows Ai Chat Ui 4`, `Windows About`, `Windows Main Window 6`, `Windows Views 30`, `Windows Color Picker`, `Windows Main Window 7`, `Windows Models 3`, `Windows Views 31`, `Windows Color Picker 2`, `Windows Export Clips`, `Windows Views 33`, `Windows Models 4`, `Windows Preferences 2`, `Windows Process Effect`, `Windows Views 34`, `Classes Path Utils`, `Classes Query 2`, `Windows Views 35`, `Windows File Properties`, `Windows Freesound Dock 3`, `Windows Profile`, `Windows Export Clips 2`, `Windows Animated Title`, `Windows Export 2`, `Windows Video Widget 2`, `Windows Views 38`, `Windows Views 39`, `Windows Views 40`, `Windows Region`, `Windows Views 42`, `Windows Process Effect 2`, `Windows Views 43`, `Windows Main Window 8`, `Windows Models 5`, `Windows Models 6`, `Windows Views 45`, `Windows Views 46`, `Windows Views 48`, `Windows Views 49`, `Windows Main Window 9`, `Windows Models 7`, `Windows Views 51`, `Windows Views 52`, `Windows Views 53`, `Windows Models 8`, `Classes Updates 4`, `Windows Main Window 10`, `Windows Models 9`, `Windows Video Widget 3`, `Windows Views 56`, `Windows Models 11`, `Windows Views 57`, `Classes Json Data 2`, `Windows Export 3`, `Windows Views 59`, `Windows Views 60`, `Classes Logger Libopenshot`, `Classes Query 4`, `Windows Title Editor 2`, `Windows Views 63`, `Windows Views 66`, `Windows Views 67`?**
  _High betweenness centrality (0.207) - this node is a cross-community bridge._
- **Why does `MainWindow` connect `Windows Main Window` to `Windows Views`, `Windows File Properties`, `Windows Profile`, `Windows Main Window 2`, `Windows Export Clips 2`, `Windows Classes`, `Windows Animated Title`, `Windows Ai Chat Ui`, `Classes Clip Resolver`, `Windows Views 39`, `Windows Title Editor`, `Windows Main Window 3`, `Windows Models`, `Windows Views 7`, `Classes App`, `Windows Video Widget`, `Windows Views 11`, `Classes Exporters`, `Windows Main Window 8`, `Windows Main Window 4`, `Windows Plan Dock Ui`, `Classes Auth Manager`, `Classes Thumbnail`, `Windows Views 46`, `Windows Views 12`, `Windows Views 49`, `Windows Views 13`, `Windows Main Window 9`, `Windows Ai Media Panel`, `Themes Manager`, `Classes Exporters 2`, `Classes Metrics`, `Windows Login Window`, `Windows Views 17`, `Classes Auto Updater`, `Classes Importers`, `Windows Main Window 10`, `Windows Export`, `Windows Main Window 5`, `Windows Main Window 11`, `Classes Title Bar`, `Windows Cutting`, `Windows Preferences`, `Windows Views 56`, `Windows Profile Edit`, `Windows Views 20`, `Windows Views 21`, `Windows Views 26`, `Windows About`, `Windows Freesound Dock`, `Windows Main Window 6`, `Classes Query 4`, `Windows Pexels Dock 2`, `Windows Main Window 7`, `Windows Models 3`, `Windows Views 31`, `Windows Views 32`, `Windows Views 34`?**
...
- **Are the 404 inferred relationships involving `get_app()` (e.g. with `._get_backend_url()` and `._emit_update_ready_signal()`) actually correct?**
  _`get_app()` has 404 INFERRED edges - model-reasoned connections that need verification._
