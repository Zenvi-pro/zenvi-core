...
- Timeline Media 22
- Windows Views 69
- ._invalidate_clip_cache_for_clip
- .reindex_video
...
- `renderMenu()` --indirect_call--> `i()`  [INFERRED]
  src/chat_ui/chat.js → src/timeline/media/js/ui-bootstrap-tpls-1.3.3.min.js
...
Cohesion: 0.04
Nodes (40): Zenvi Backend API Client.  Drop-in replacement for direct AI class imports. Th, @file  @brief This file creates the QApplication, and displays the main window, @file  @brief This file generates the path for a project's assets  @author Jonat, @file  @brief This file loads and saves settings (as JSON)  @author Noah Figg <e, @file  @brief Process project data, scaling keyframe X coordinates by the given, @file  @brief This file sets the default logging settings  @author Noah Figg <, Adjust the minimum log level written to our logfile, Adjust the minimum log level for output to the terminal (+32 more)

### Community 1 - "Classes Tool Handlers"
Cohesion: 0.04
Nodes (108): Get absolute file path of file, add_clip_to_timeline(), add_marker(), add_stock_media_to_project(), add_track(), add_transition_between_clips(), add_transition_to_clip(), add_tts_audio_to_timeline() (+100 more)
...
Cohesion: 0.03
Nodes (34): get_app(), Get the current QApplication instance of OpenShot, get_audio_data(), Get a Clip object form libopenshot, and grab audio data         For for the give, CosmicTheme, Toggle the play icon between play and pause., Handle when playback is started, Handle when playback is paused (+26 more)

### Community 5 - "Windows Views 2"
Cohesion: 0.05
Nodes (37): BackgroundPainter, QPainter, QRectF, @file  @brief Painter for the timeline background gradient.  @author Jonathan Th, BasePainter, @file  @brief Base painter helpers for the QWidget timeline backend.  @author Jo, Return ``(width, height)`` of *pixmap* in logical units., Return *pixmap* scaled to the requested logical size. (+29 more)

### Community 6 - "Windows Classes"
Cohesion: 0.10
Nodes (12): @file  @brief This file can easily query Clips, Files, and other project data, @file  @brief This file has code to generate audio waveform data structures  @au, # NOTE: we also have the average RMS value calculated, although we do, @file  @brief This file loads the Addtotimeline dialog (i.e add several clips in, # TODO: Determine why this even happens, as it shouldn't be possible, @file  @brief This file loads the Video Export dialog (i.e where is all preferen, @file  @brief This file contains the clip properties model, used by the properti, # TODO: Determine why c is occasional = None (+4 more)
...
Cohesion: 0.06
Nodes (54): addReasoningStep(), adjustTextareaHeight(), allAnswered(), applyZenviWebKitInlineTheme(), buildOption(), cancelRequest(), clearChat(), clearReasoningStep() (+46 more)

### Community 11 - "Classes Clip Utils"
Cohesion: 0.08
Nodes (55): Fraction, _as_mapping(), _clamp_basic_timing(), _clamp_curve_start(), _clamp_time_points(), clamp_timing_to_media(), _clip_has_single_image(), _clip_id() (+47 more)
...
Cohesion: 0.20
Nodes (6): dict_to_style(), @file  @brief Utility functions for manipulating SVG style attributes  @author F, Turn an exploded style dictionary back into a string, set_if_existing(), sets the QFont properties to all SVG: TEXT and TSPAN nodes, sets the background color
...
Cohesion: 0.23
Nodes (5): EmojisListView, QListView, Filter emoji with proxy class, A QListView QWidget used on the main window, Override startDrag method to display custom icon
...
Cohesion: 0.09
Nodes (13): reset_repeat(), Show a waveform for all selected clips, Callback for split audio context menus, Callback for the layout context menus, Callback for the animate context menus, Add a Point to a Keyframe dict. Always remove existing points,         if any c, Callback for fade context menus, Callback for volume context menus (+5 more)

### Community 25 - "Windows Views 9"
Cohesion: 0.09
Nodes (10): Clip, This class allows Clips to be queried, updated, and deleted from the project dat, Take any arguments given as filters, and find a list of matching objects, ClipInteractionMixin, Begin a drag operation on one or many selected clips/transitions., Apply identical horizontal/vertical deltas to every dragged item., Publish the current drag overrides (clips + transitions) to the overview., Return a QRectF encompassing all currently-selected clips and transitions. (+2 more)

### Community 26 - "Classes App"
Cohesion: 0.07
Nodes (22): QApplication, get_settings(), _install_windows_qfiledialog_workaround(), OpenShotApp, _qt_message_handler(), Get a reference to the app's settings object, Store and later display an error encountered during setup, Create an error message object, populated with details (+14 more)

### Community 27 - "Windows Video Widget"
Cohesion: 0.06
Nodes (24): QPointF, QWidget, Update a keyframe property to a new value, adding or updating keyframes as neede, Return (QTransform, unpacked props, originScreenPt) for a clip/effect box., Update a keyframe property to a new value, adding or updating keyframes as neede, Return clip-relative frame clamped between trimmed start/end., Signal to refresh viewport (i.e. a property might have changed that effects the, Handle the transform signal when it's emitted. Supports multiple clip IDs. (+16 more)

### Community 28 - "Classes Ai Metadata Utils"
Cohesion: 0.09
Nodes (35): adjust_scene_descriptions_for_subclip(), apply_metadata_to_clip_data(), build_summary_preview(), clip_metadata_is_valid(), collect_scene_descriptions_for_baked_segment(), _filter_scenes_in_window(), _filter_tags_for_window(), filter_tags_string_for_window() (+27 more)

### Community 29 - "Windows Views 10"
Cohesion: 0.07
Nodes (12): QSignalTransition, QWidget, Clear all timeline selections and keyframe highlights., Enable or disable snapping mode., Enable or disable razor tool mode., Enable or disable timing (retime) mode., Placeholder due to webview compatibility, Placeholder due to webview compatibility (+4 more)

### Community 30 - "Windows Views 11"
Cohesion: 0.10
Nodes (9): QGridLayout, QPixmap, QWidget, Embed the project-files QListView as the top section and restyle it         to, Search both stock providers for ``query`` and show the two sections., Hide and empty the stock sections (project files stay visible)., Import the downloaded file into Project Files (re-tag if already present)., Project files + stock footage + stock music, sharing one scroll area. (+1 more)

### Community 31 - "Classes Exporters"
Cohesion: 0.10
Nodes (22): _clip_media_path(), _db_to_volume(), export_edl(), _fmt_percent(), _fmt_value(), _interp_name(), _is_drop_frame(), @file  @brief This file is used to generate an EDL (edit decision list) export (+14 more)

### Community 32 - "Windows Main Window 4"
Cohesion: 0.05
Nodes (23): get_icon(), Get either the current theme icon or fallback to default theme (for custom icons, Get a list of all dockable widgets, Remove all dockable widgets on main screen, Add all dockable widgets to the same dock area on the main screen, Float or Un-Float all dockable widgets above main screen, Show all dockable widgets on the main screen, Freeze/unfreeze a dock widget on the main screen. (+15 more)
...
Cohesion: 0.08
Nodes (26): BaseHTTPRequestHandler, HTTPServer, _ensure_thumb_dir(), _generate_thumbnail_ffmpeg(), GenerateThumbnail(), httpThumbnailException, httpThumbnailHandler, httpThumbnailServer (+18 more)

### Community 39 - "Windows Views 12"
Cohesion: 0.12
Nodes (11): QObject, @file  @brief Lightweight thumbnail request manager for the QWidget timeline bac, Drop any pending requests., Stop the worker thread., Worker object that resolves thumbnail paths on a background thread., Queue a thumbnail request., Discard any pending thumbnail work., Qt helper that forwards thumbnail requests to a worker thread. (+3 more)
...
Cohesion: 0.11
Nodes (13): QObject, Manage and present a list of tutorial dialogs, Process and show the first non-completed tutorial, Get an object from the main window by object id, Mark the current tip completed, and show the next one, Hide the current tip, and don't show anymore, Close any open tutorial dialogs, Disconnect from all signals, and shutdown tutorial manager (+5 more)
...
Cohesion: 0.20
Nodes (14): get_backend_client(), Get the singleton backend client (refreshed when backend URL changes)., charge_operation_on_success(), check_operation(), download_freesound_music_tool(), download_pexels_video_tool(), get_project_catalog(), import_stock_media() (+6 more)

### Community 47 - "Classes Exporters 2"
Cohesion: 0.12
Nodes (26): _append_link(), _apply_rate_settings(), _apply_timecode_settings(), _clip_file_info(), _displayformat(), export_xml(), _format_ratio(), _format_timebase() (+18 more)

### Community 48 - "Classes Metrics"
Cohesion: 0.07
Nodes (24): _base_event_params(), _build_event(), _d(), Build a GA4 event payload., Track a GUI screen being shown, Track an error has occurred, Track application session start/end., Send anonymous GA4 Measurement Protocol events over HTTP. (+16 more)
...
Cohesion: 0.12
Nodes (8): LoginWindow, _PasswordWorker, _PollWorker, QDialog, QObject, Zenvi login window.  Primary flow  — browser OAuth:   Opens ZENVI_WEBSITE/log, Runs AuthManager.poll_for_session() in a QThread and bridges to Qt signals., Calls sign_in or sign_up in a QThread.

### Community 52 - "Windows Preview Thread"
Cohesion: 0.14
Nodes (10): PlayerWorker, QT Player Worker Object (to preview video on a separate thread), Check if any audio devices initialization errors, default sample rate, and curre, This method starts the video player, Preview a certain frame, Refresh a certain frame, Load a media file into the video player, Start playing the video player (+2 more)
...
Cohesion: 0.08
Nodes (13): QWidget, Allow the slider to shrink horizontally when space is limited., GUI-thread slot: adopt the latest committed geometry and repaint., Capture mouse press event, Capture mouse release event, Set min/max limits on the bounds of the handles (to prevent invalid values), Calculate the width of the scrollbar handle (i.e. selection width), Callback when position is changed (+5 more)
...
Cohesion: 0.13
Nodes (11): @file  @brief Background auto-updater that checks GitHub releases and downloads, @file  @brief This file loads the clip cutting interface (quickly cut up a clip, # NOTE: Do NOT inject file.data into the reader via SetJson., @file  @brief This file loads the main window (i.e. the primary user-interface), # TODO: Figure out why this is needed for a backup recovery to correctly show up, @file  @brief This file contains the preview thread, used for displaying previe, # TODO: Remove this hack and really determine what's blocking the main thread, @file  @brief This file loads the UI for selecting a region of a video (rectangl (+3 more)
...
Cohesion: 0.13
Nodes (12): project, @file  @brief This file is for legacy support of OpenShot 1.x project files  @au, This is the main project class that contains all     the details of a project, s, @file  @brief Helper for horizontal snapping.  @author Jonathan Thomas <jonathan, Return generic snap targets converted to seconds for keyframe drags., Return (diff, target, reused_active, tolerance_px) for a given cursor position., Compute horizontal snap offsets for dragged clips and transitions., Return adjusted delta in seconds for horizontal snapping. (+4 more)

### Community 61 - "Classes Query"
Cohesion: 0.04
Nodes (29): QItemDelegate, Effect, File, Save the object back to the project data store, Get the translated display title of this item, Save the object back to the project data store, This class allows Files to be queried, updated, and deleted from the project dat, Save the object back to the project data store (+21 more)
...
Cohesion: 0.10
Nodes (11): Lazy-create a requests.Session., Remove a temp upload on the backend after tag/index complete., Check if the backend is running., Get the default model ID., Clear a chat session., Send a chat message via WebSocket with tool delegation support.          Each, HTTP/WebSocket client for the Zenvi backend API., Get backend URL from app settings or environment.          Supports multi-URL (+3 more)

### Community 68 - "Classes Importers 2"
Cohesion: 0.13
Nodes (16): get_media_type(), is_audio_path(), is_image(), @file  @brief This file is used to determine if an extension is an image format, Check a File object if the file extension is a known image format, True when path ends with a known pure-audio extension., Check a File object and determine the media type (video, image, audio), create_clip() (+8 more)
...
Cohesion: 0.12
Nodes (4): Callback for locking a track, Callback for unlocking a track, @file  @brief Track toolbar and menu helpers.  @author Jonathan Thomas <jonathan, TrackInteractionMixin

### Community 76 - "Windows Views 21"
Cohesion: 0.05
Nodes (22): ClipboardManager, @file  @brief This file is responsible for serializing copy/paste clipboard data, Manages clipboard operations for QueryObjects or lists of QueryObjects, Converts QueryObject or list of QueryObjects to QMimeData.         Handles both, QueryObject, Return a cached, detached copy of child, clearing cache when project changes., Take any arguments given as filters, and find a list of matching objects, Take any arguments given as filters, and find the first matching object (+14 more)

### Community 77 - "Windows Pexels Dock"
Cohesion: 0.13
Nodes (5): Any, QFrame, QPixmap, A single result card: thumbnail + duration badge + hover overlay., _VideoCard

### Community 78 - "Windows Views 22"
Cohesion: 0.10
Nodes (6): BlenderListView, QListView, Callback when the user chooses a color in the dialog, Get new value of preview slider, and start timer to Render frame, Show a friendly error message regarding the blender executable or version., A ListView QWidget used on the animated title window
...
Cohesion: 0.16
Nodes (4): @file  @brief This file is for legacy support of OpenShot 1.x project files  @au, This class contains methods to simply displaying time codes, timeline, Handle context menu interaction on an effect badge.
...
Cohesion: 0.09
Nodes (11): Any, Search Pexels for stock videos., Search Freesound for stock music and sound effects., List all known LLM models.          Rows carry model_id/display_name plus pick, Get conversation history., Fetch agent/tool telemetry events for diagnosing loops and bottlenecks., Search for clips matching a query., Generate a video from a text prompt (Kling O1 Pro via Runware).          Suppo (+3 more)
...
Cohesion: 0.21
Nodes (14): cleanup_chunk_dir(), extract_chunk(), extract_chunks(), _ffmpeg_run(), _is_full_file_chunk(), _looks_like_audio_path(), _probe_duration(), Any (+6 more)

### Community 93 - "Windows About"
Cohesion: 0.07
Nodes (20): About, Changelog, Credits, License, parse_changelog(), QDialog, @file  @brief This file loads the About dialog (i.e about Openshot Project)  @, Handle right-click context menu. (+12 more)
...
Cohesion: 0.12
Nodes (8): PlayheadMixin, @file  @brief Playhead movement and rendering helpers.  @author Jonathan Thomas, Return QRectF describing the draggable portion of the playhead., Return True if *pos* intersects the draggable playhead handle., Callback when position is changed, Callback when play button is clicked, Connect playback signals, Return QRectF describing the full rendered playhead icon.
...
Cohesion: 0.15
Nodes (7): BlockPaintFilter, draw_checkerboard(), PickingDialog, PreviewFrameFilter, QDialog, Draw a checkerboard pattern for transparent backgrounds., Intercepts paint events on the QColorDialog’s preview frame,     draws a checker

### Community 101 - "Windows Freesound Dock 2"
Cohesion: 0.15
Nodes (10): _DownloadWorker, QObject, QRunnable, Freesound stock-music/SFX search dock for Zenvi.  Search bar (QLineEdit + magn, Runs freesound_search on a background thread., Downloads a single Freesound MP3 preview on a background thread., Fetches a waveform image URL and emits the result via a signal carrier., _SearchWorker (+2 more)
...
Cohesion: 0.21
Nodes (5): FilesModel, QObject, Attach AI metadata to a file object (does not save)., Queue a file for background indexing/summarize with bounded concurrency., Fire-and-forget background indexing/summarize for an already-saved file.
...
Cohesion: 0.16
Nodes (5): QColorDialog, ColorPicker, Find the built-in preview frame and install a filter on it., Handle window resize events to update preview frame filter., Track when the current color changes (hover or selection).

### Community 106 - "Windows Export Clips"
Cohesion: 0.22
Nodes (13): _pathurl_to_path(), Convert a Final Cut pathurl value into a filesystem path., absolute_media_path(), absolute_path_from_export(), _project_file_path(), _project_folder(), @file  @brief Helpers for resolving media paths to absolute/relative forms  @a, Return path relative to export folder when possible. (+5 more)
...
Cohesion: 0.10
Nodes (11): Converts QMimeData back into the original object (QueryObject or list of QueryOb, This class allows Transitions (i.e. timeline effects) to be queried, updated, an, Take any arguments given as filters, and find a list of matching objects, Get the translated display title of this item, Transition, Persist all moved clips/transitions and refresh geometry., Finalize box-select: add items intersecting the selection rectangle., Get a unique id (used for generating a transaction id for the undo/redo system) (+3 more)
...
Cohesion: 0.21
Nodes (7): KeyframeScaler, This factory class produces scaler objects which, when called,     will apply th, Scale value by some factor, except for 1 (leave that alone), To keep keyframes at the same time in video,         update frame numbers to the, Process all the dict sub-members of the current dict, Apply the stored scaling factor to a project data dict, Store the scale factor assigned to this instance
...
Cohesion: 0.09
Nodes (9): QStyledItemDelegate, FileCardDelegate, FilesListView, QListView, A ListView QWidget used on the main window, Override startDrag method to display custom icon, Filter files with proxy class, Renders a quick-action overlay (Preview · Add · Remove) when a     thumbnail is (+1 more)

### Community 116 - "Windows Login Window 2"
Cohesion: 0.27
Nodes (6): padNumber(), Pad number a specific # of characters, Convert # of seconds (float) to parts of time (dict), secondsToTime(), QPainter, RulerPainter
...
Cohesion: 0.27
Nodes (3): FreesoundDock, QDockWidget, Freesound stock-music/SFX search dock.
...
Cohesion: 0.15
Nodes (4): Any, QFrame, A single result card: waveform + name + duration badge + hover overlay., _SoundCard

### Community 130 - "Windows Views 36"
Cohesion: 0.10
Nodes (12): ChangelogFilterProxyModel, ChangelogModel, ChangelogStandardItemModel, QSortFilterProxyModel, QStandardItemModel, @file  @brief This file contains the git changelog model, used by the about wind, Proxy class used for sorting and filtering changelog entries, ChangelogTreeView (+4 more)
...
Cohesion: 0.22
Nodes (10): effect_color_hex(), effect_color_qcolor(), _effect_type_name(), Any, QColor, @file  @brief Shared helpers for timeline color mappings.  @author Jonathan Thom, Return the preferred hex color string for *effect*., Return a QColor matching the preferred color for *effect*. (+2 more)

### Community 133 - "Windows Export Clips 2"
Cohesion: 0.18
Nodes (5): PreviewParent, QObject, Class which communicates with the PlayerWorker Class (running on a separate thre, This method is invoked by the UpdateManager each time a change happens (i.e Upda, Set the speed of the video player
...
Cohesion: 0.07
Nodes (15): Set up model/view classes for MainWindow, EffectsModel, EffectsProxyModel, QObject, QSortFilterProxyModel, Filter for common transitions and text filter, EmojisModel, EffectsTreeView (+7 more)

### Community 142 - "Windows Video Widget 2"
Cohesion: 0.20
Nodes (4): Generate a new, unique folder name to contain Blender frames, Enable all controls on interface, Init the slider and preview frame label to the currently selected animation, Build a dictionary of all animation settings and properties from XML

### Community 143 - "Windows Views 38"
Cohesion: 0.20
Nodes (6): QObject, Timer is ready to Render frame, Cancel the current render, if any, Background Worker Object (to run the Blender commands), Worker's Render method which invokes the Blender rendering commands, Worker

### Community 144 - "Windows Views 39"
Cohesion: 0.20
Nodes (3): Disable all controls on interface, Return a dictionary of project related settings, needed by the Blender python sc, Render an images sequence of the current template using Blender 2.62+ and the
...
### Community 146 - "Classes Frame Extractor"
Cohesion: 0.10
Nodes (4): Return list of selected ids matching item_type, Invoke razor slicing when the razor tool is enabled and an item is clicked., Show appropriate context menu for the position. Returns True if handled., Convert an x position in widget coordinates to timeline seconds.
...
Cohesion: 0.29
Nodes (4): QPainter, Return cached pixmap for the visible portion of a transition., Clear cached rendered transition pixmaps., TransitionPainter
...
Cohesion: 0.20
Nodes (8): _get_dispatcher(), _MainThreadDispatcher, QObject, Re-run Pegasus audiovisual summary for an already-indexed project file.      R, Singleton helper that runs callables on the Qt main (GUI) thread., Headless fallback when PyQt5 is unavailable., Return (and lazily create) the singleton main-thread dispatcher., resummarize_project_file()
...
Cohesion: 0.11
Nodes (14): QObject, QSortFilterProxyModel, QStandardItemModel, @file  @brief This file contains the titles model, used by the title editor wind, Proxy class used for sorting and filtering model data, Sort titles model by a column at runtime, TitleFilterProxyModel, TitleRoles (+6 more)

### Community 163 - "Windows Pexels Dock 3"
Cohesion: 0.13
Nodes (12): _DownloadWorker, QObject, QRunnable, Pexels stock-video search dock for Zenvi.  Search bar (QLineEdit + magnifying-, Runs pexels_search on a background thread., Downloads a single Pexels video on a background thread., # NOTE: do NOT touch self._dl_threads/_dl_workers here — the QThread, Fetches a thumbnail URL and emits the result via a signal carrier. (+4 more)
...
### Community 165 - "Windows Views 45"
Cohesion: 0.28
...
Cohesion: 0.40
Nodes (3): Marker, This class allows Markers to be queried, updated, and deleted from the project d, Take any arguments given as filters, and find a list of matching objects
...
Cohesion: 0.22
Nodes (3): Toggle between zooming to the entire timeline and the previous zoom, Callback for resize event timer (to delay the resize event, and prevent lots of, Set the current zoom factor (do not clamp width here — backend owns authoritativ

### Community 170 - "Classes Logger"
Cohesion: 0.22
Nodes (5): object, Route stdout and stderr to logger (custom handler), Custom class to log all stdout and stderr streams (from libopenshot / and other, reroute_output(), StreamToLogger
...
Cohesion: 0.29
Nodes (4): QWidget, Process click events on tutorial. Especially useful when tutorial messages are p, A customized QWidget used to instruct a user how to use a certain feature, TutorialDialog
...
Cohesion: 0.25
Nodes (8): _download_and_import_one(), _download_motion_graphics_file(), fetch_motion_graphics_video(), _motion_graphics_cleanup_storage(), Download a Supabase mp4 to a fresh temp path. Returns (dest_path, size_mb)., Download one Supabase mp4 and import it as a project file.      Returns (file_, Best-effort DELETE {HYPERFRAMES_URL}/cleanup. Non-critical — failures are logged, Import rendered HyperFrames motion/demo segments into the project files panel.
...
Cohesion: 0.22
Nodes (6): Callback for max sized change (i.e. max size of video widget), This class syncs changes from the timeline to libopenshot, This method is invoked by the UpdateManager each time a change happens (i.e Upda, TimelineSync, Interface for classes that listen for changes (insert, update, and delete)., UpdateInterface
...
Cohesion: 0.14
Nodes (8): _ConditionalTransition, QObject, TimelineEvents, EffectInteractionMixin, @file  @brief Effect badge and drop handling helpers.  @author Jonathan Thomas <, @file  @brief QWidget-based timeline widget composed from focused mixins.  @auth, @file  @brief Transition-specific geometry helpers.  @author Jonathan Thomas <jo, TransitionInteractionMixin

### Community 193 - "Windows Views 56"
Cohesion: 0.46
Nodes (3): QPainter, QRectF, TrackPainter
...
### Community 201 - "Windows Views 58"
Cohesion: 0.33
...
Cohesion: 0.33
Nodes (3): Get the file ID of the current files-view item, or the first selection, Get the File object for the current files-view item, or the first selection, Table cell change event - when tags are updated on a file
...
Cohesion: 0.40
Nodes (4): Any, Upload a local chunk file to a Gemini Files resumable upload URL., PUT/finalize bytes to a Gemini resumable upload URL.      Returns ({name, uri,, upload_file_to_gemini_resumable()
...
### Community 211 - ".detect_font"
Cohesion: 0.29
Nodes (5): BackendIndexingWorker, QThread, Close the active HTTP session to unblock any pending request., Called at app quit — interrupt any pending HTTP requests, then wait briefly., Background worker: Gemini index + Flash audiovisual summary.
...
### Community 215 - ".Paste_Triggered"
Cohesion: 0.40
Nodes (5): find_missing_file(), _get_dialog_parent(), @file  @brief This file is used to find missing files when opening or importing, Return main window as dialog parent so dialogs stay on top and are clickable., Find a missing file name or file path, and return valid path.     If parent is N
...
### Community 224 - "_normalized_to_center_pixels"
Cohesion: 0.29
Nodes (3): LoggerLibOpenShot, Thread, @file  @brief This file connects to libopenshot and logs debug messages (if deb

### Community 226 - "Windows Title Editor 2"
Cohesion: 0.08
Nodes (19): Explode an SVG node style= attribute string into a dict representation, style_to_dict(), QColor, QDialog, Checks for a valid font name and returns a QFont object.         If the requeste, Display pixmap of SVG on UI thread, Set temp file path & make copy of template, Detect font from SVG: TEXT and TSPAN nodes (or use fallback font) (+11 more)

### Community 227 - "Windows Views 63"
Cohesion: 0.07
Nodes (20): QMenu, @file  @brief This file contains the changelog treeview, used by the about windo, @file  @brief This file contains the credits treeview, used by the about window, @file  @brief This file contains the effects file listview, used by the main win, @file  @brief This file contains the effects file treeview, used by the main win, @file  @brief This file contains the project file listview, used by the main win, @file  @brief This file contains the project file treeview, used by the main win, @file  @brief This file creates a styled QMenu (which supports border radius and (+12 more)

### Community 229 - "Windows Views 65"
Cohesion: 0.13
Nodes (10): AddToTimeline, QDialog, Callback for move up button click, Callback for move up button click, Handle callback for image length being changed, Calculate the total length of what's about to be added to the timeline, Cancel button clicked, Add To timeline Dialog (+2 more)

### Community 234 - "Timeline Media 13"
Cohesion: 0.50
Nodes (4): expectSync(), leverageNative(), returnTrue(), safeActiveElement()

### Community 235 - "Windows Export 4"
Cohesion: 0.33
Nodes (3): EmojiStandardItemModel, QStandardItemModel, @file  @brief This file contains the emoji model, used by the main window  @auth

### Community 238 - "Windows Views 67"
Cohesion: 0.13
...
### Community 283 - ".reindex_video"
Cohesion: 0.22
Nodes (5): Re-index via direct TwelveLabs presigned upload., Thread-safe session for parallel upload + indexing requests., Index via editor-side chunks + Gemini Files direct upload (no backend media stor, Poll /indexing/job/{job_id} until the job finishes or max_wait seconds pass., guess_mime()
...
- **Why does `get_app()` connect `Windows Main Window 2` to `Windows Views`, `Classes Tool Handlers`, `Windows Main Window`, `Windows Views 2`, `Windows Views 3`, `Classes Project Data`, `Windows Views 4`, `Classes Clip Utils`, `Windows Ai Chat Ui`, `Windows Views 6`, `Classes Clip Resolver`, `Windows Title Editor`, `Windows Main Window 3`, `Windows Models`, `Windows Views 7`, `Windows Views 8`, `Windows Views 9`, `Classes App`, `Windows Video Widget`, `Windows Views 10`, `Windows Views 11`, `Classes Exporters`, `Windows Main Window 4`, `Classes Thumbnail`, `Classes Twelvelabs Match`, `Windows Views 13`, `Windows Ai Media Panel`, `Classes Tool Handlers 2`, `Classes Exporters 2`, `Classes Metrics`, `Windows Preview Thread`, `Windows Views 17`, `Classes Auto Updater`, `Classes Importers`, `Windows Views 19`, `Classes Query`, `Windows Export`, `Windows Main Window 5`, `Classes Api Client`, `Classes Importers 2`, `Windows Cutting`, `Windows Preferences`, `Windows Profile Edit`, `Classes Ui Util`, `Windows Views 20`, `Windows Views 21`, `Windows Pexels Dock`, `Windows Views 22`, `Windows Views 23`, `Windows Views 24`, `Windows Views 27`, `Windows Views 28`, `Windows Ai Chat Ui 4`, `Windows About`, `Windows Freesound Dock`, `Windows Views 30`, `Windows Color Picker`, `Windows Main Window 7`, `Windows Models 3`, `Windows Views 31`, `Windows Color Picker 2`, `Windows Export Clips`, `Windows Views 33`, `Windows Models 4`, `Windows Process Effect`, `Windows Views 34`, `Windows Login Window 2`, `Classes Exporters 3`, `Classes Path Utils`, `Windows Views 35`, `Windows File Properties`, `Windows Freesound Dock 3`, `Windows Views 36`, `Windows Profile`, `Windows Export Clips 2`, `Windows Animated Title`, `Windows Export 2`, `Windows Views 38`, `Windows Views 39`, `Windows Views 40`, `Classes Frame Extractor`, `Windows Views 42`, `Windows Process Effect 2`, `Windows Views 43`, `Windows Main Window 8`, `Windows Models 5`, `Windows Models 6`, `Windows Views 44`, `Windows Views 46`, `Windows Views 47`, `Windows Views 48`, `Windows Views 49`, `Classes Settings 2`, `Windows Models 7`, `Windows Views 51`, `Windows Views 52`, `Windows Models 8`, `Classes Updates 4`, `Classes Updates 5`, `Windows Main Window 10`, `Windows Models 9`, `Classes Title Bar`, `Windows Models 10`, `Windows Models 11`, `Windows Views 57`, `Windows Views 58`, `Windows Views 59`, `.Paste_Triggered`, `_normalized_to_center_pixels`, `Windows Models 13`, `Windows Title Editor 2`, `Windows Views 63`, `Windows Views 65`, `Windows Preferences 3`, `Windows Views 66`, `Windows Views 67`?**
  _High betweenness centrality (0.228) - this node is a cross-community bridge._
- **Why does `MainWindow` connect `Windows Main Window` to `Windows File Properties`, `Windows Profile`, `Windows Main Window 2`, `Windows Export Clips 2`, `Windows Animated Title`, `Windows Ai Chat Ui`, `Windows Export 2`, `Classes Frame Extractor`, `Windows Main Window 3`, `Windows Models`, `Windows Views 7`, `Windows Views 9`, `Classes App`, `._invalidate_clip_cache_for_clip`, `Windows Views 43`, `Windows Video Widget`, `Windows Views 11`, `Classes Exporters`, `Windows Main Window 4`, `Windows Plan Dock Ui`, `Classes Auth Manager`, `Windows Views 44`, `Classes Thumbnail`, `Windows Views 47`, `Windows Views 48`, `Windows Views 13`, `Windows Ai Media Panel`, `Themes Manager`, `Classes Metrics`, `Windows Views 53`, `Windows Login Window`, `Windows Views 17`, `Windows Models 2`, `Classes Auto Updater`, `Classes Importers`, `Classes Query`, `Windows Main Window 10`, `Windows Export`, `Windows Main Window 11`, `Classes Importers 2`, `Windows Cutting`, `Windows Preferences`, `Windows Profile Edit`, `Classes Ui Util`, `Windows Views 20`, `Windows About`, `Windows Freesound Dock`, `Windows Pexels Dock 2`, `Windows Title Editor 2`, `Windows Views 63`, `Windows Views 65`, `Windows Main Window 7`, `Windows Models 3`, `Windows Views 31`, `Windows Preferences 3`, `Windows Views 33`, `Windows Views 32`, `Windows Views 34`, `Classes Query 2`?**
