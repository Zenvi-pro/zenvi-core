...
minutes to hours for a real project. Chat/MCP tool calls dispatch through
execute_tool() -> _run_on_main_thread(), which defaults to a 30s wait; a
real long export would previously report "Export failed" (TimeoutError)
to the agent while still rendering in the background. These tests cover
both the dispatch wiring (right tool -> right timeout) and the real
cross-thread timeout mechanics (using small durations standing in for the
real 30s / 6h, so the tests stay fast).
"""
...
def test_export_video_tool_gets_the_extended_timeout(monkeypatch):
    ...
    assert seen_timeouts == [6 * 60 * 60], (
        "export_video_tool must use the extended timeout, not the 30s default"
    )
...
def test_real_dispatch_survives_past_the_old_default_with_the_override(monkeypatch):
    ...
    succeed once export_video_tool's own timeout override applies, and a
    tool without the override must still time out under the same slow
    handler -- proving the override, not some other change, is what saves
    the export path."""
    ...
    # waits at most this long. export_video_tool's override (patched down
    # from 6h to comfortably above the slow handler) must let it through.
    monkeypatch.setattr(th, "_MAIN_THREAD_TIMEOUTS", {"export_video_tool": 2.0})
    ...
    def slow_handler(**_kw):
        time.sleep(slow_handler_delay)
        return "done"

    monkeypatch.setitem(th.TOOL_HANDLERS, "export_video_tool", slow_handler)
    monkeypatch.setitem(th.TOOL_HANDLERS, "some_other_slow_tool", slow_handler)
    ...
    # Control: a tool without export_video_tool's override still times out
    ...
    def call_other():
        try:
            return th._run_on_main_thread(
                lambda: th.TOOL_HANDLERS["some_other_slow_tool"](),
    ...
    # export_video_tool's own override lets the same slow handler finish.
    export_box = {}
    _run_and_pump(
        lambda: th.execute_tool("export_video_tool", {}), export_box, "export", join_timeout=5
    )
    assert export_box.get("export") == "done", (
        "export_video_tool's extended timeout should let the slow handler finish"
    )
