...
# --------------------------------------------------------------------------
# Watch refines the cut, but speech safety still wins
# --------------------------------------------------------------------------

def _watching(result):
    return patch.object(tool_handlers, "_watch_source_window", return_value=result)


def test_a_watched_cut_is_still_snapped_off_speech():
    """Chain order: vision refine -> speech snap -> frame snap.

    A vision-chosen peak is no more allowed to land mid-word than an index one.
    """
    watched = {"matched": True, "cut_source": 5.4, "in_source": 5.0,
               "out_source": 8.0, "used_fallback": False, "confidence": 0.9}
    with _watching(watched):
        cut = tool_handlers._watch_source_window("/m/a.mp4", 0.0, 30.0, "q")["cut_source"]
    with _patched(CUES):
        snapped, moved = tool_handlers._snap_cut_off_speech("C1", cut)
    assert snapped == pytest.approx(5.0)
    assert moved is True


def test_a_failed_watch_leaves_the_original_cut_untouched():
    with _patched(CUES):
        fallback = tool_handlers._watch_source_window(
            "/nonexistent/file.mp4", 10.0, 20.0, "q", fallback_cut=12.0,
        )
    assert fallback["matched"] is False
    assert fallback["used_fallback"] is True
    assert fallback["cut_source"] == pytest.approx(12.0)


def test_the_watch_helper_never_raises_on_a_broken_backend():
    broken = MagicMock()
    broken.get_backend_client.side_effect = RuntimeError("no backend")
    with patch.dict(sys.modules, {"classes.api_client": broken}):
        out = tool_handlers._watch_source_window("/m/a.mp4", 1.0, 5.0, "q", fallback_cut=2.0)
    assert out["used_fallback"] is True
    assert out["cut_source"] == pytest.approx(2.0)


def test_the_watch_helper_clamps_a_model_answer_into_the_window():
    client = MagicMock()
    client.watch_window.return_value = {
        "matched": True, "used_fallback": False,
        "cut_source": 999.0, "in_source": -5.0, "out_source": 999.0, "confidence": 0.5,
    }
    with patch.dict(sys.modules, {
        ...
        out = tool_handlers._watch_source_window("/m/a.mp4", 10.0, 20.0, "q")
    assert 10.0 <= out["cut_source"] <= 20.0
    assert 10.0 <= out["in_source"] < out["out_source"] <= 20.0
